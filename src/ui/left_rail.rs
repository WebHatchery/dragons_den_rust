...
/// Warm gold used across the rail's click/income accents.
const GOLD: Color = Color::new(0.95, 0.72, 0.35, 1.0);
...
pub fn draw(ctx: &GameplayCtx<'_>, rect: Rect, actions: &mut Vec<UiAction>) {
    ...
    draw_surface(
        art,
        &SurfaceStyle::new(Color::new(0.16, 0.13, 0.06, 1.0))
            .with_border(1.0, Color::new(0.95, 0.72, 0.35, 0.55)),
    );
...
fn draw_click_button(
    ...
    draw_circle_lines(cx, cy, radius, 3.0, Color::new(GOLD.r, GOLD.g, GOLD.b, 0.9));
...
fn draw_footer(ctx: &GameplayCtx<'_>, content: Rect) {
    ...
    draw_ui_text_ex(
        ...
        content.x,
        base + 26.0,
        TextStyle::new(20.0, dark::POSITIVE).params(),
    );
    draw_ui_text_ex(
